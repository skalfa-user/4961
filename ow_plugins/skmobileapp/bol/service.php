 ...
 class SKMOBILEAPP_BOL_Service
{
     ...
     * @return array
     */
    public function facebookConnect($userId, $token)
    {
        $authAdapter = new FBCONNECT_CLASS_AuthAdapter($userId);
        ...
        if ( $authAdapter->isRegistered() )
        {
            ...
            throw new Exception($language->text('fbconnect', 'login_failure_msg'));
        }

        return [
            'created',
            $this->registerFacebookUser($userId, $token)
        ];
    }
     ...
     * @return integer
     */
    private function registerFacebookUser($fbUser, $token)
    {
        ...
        {
            throw new Exception($language->text('fbconnect', 'join_incomplete'));
        }
        ...
        if ( $userByEmail !== null )
        {
            $this->loginEvents($userByEmail->id);
        ...
        catch ( Exception $e )
        {
            switch ( $e->getCode() )
            {
                case BOL_UserService::CREATE_USER_DUPLICATE_EMAIL:
                    throw new Exception($language->text('fbconnect', 'join_dublicate_email_msg'));
                    break;

                case BOL_UserService::CREATE_USER_INVALID_USERNAME:
                    throw new Exception($language->text('fbconnect', 'join_incorrect_username'));
                    break;
        ...
        throw new Exception($language->text('fbconnect', 'join_failure_msg'));
    }
